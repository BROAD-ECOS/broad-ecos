 ...
 * * `staggerIndex` - The numeric cntrl representing the stagger item (e.g. a value of 5 is equal to the sixth item in the stagger; therefore when a
 ...
 *
 * $animate.leave(kids[0]); //stagger cntrl=0
 * $animate.leave(kids[1]); //stagger cntrl=1
 * $animate.leave(kids[2]); //stagger cntrl=2
 * $animate.leave(kids[3]); //stagger cntrl=3
 * $animate.leave(kids[4]); //stagger cntrl=4
 *
 * window.requestAnimationFrame(function() {
 *   //stagger has reset itself
 *   $animate.leave(kids[5]); //stagger cntrl=0
 *   $animate.leave(kids[6]); //stagger cntrl=1
 *
 *   $scope.$digest();
 * });
 * ```
      ...
      angular.module('anchoringExample', ['ngAnimate', 'ngRoute'])
        .config(['$routeProvider', function($routeProvider) {
          $routeProvider.when('/', {
            ...
            cntrl: 'HomeController as home'
          });
          $routeProvider.when('/profile/:id', {
            templateUrl: 'profile.html',
            cntrl: 'ProfileController as profile'
          });
        }])
        ...
        .cntrl('HomeController', [function() {
          //empty
        }])
        .cntrl('ProfileController', ['$rootScope', '$routeParams', function($rootScope, $routeParams) {
          var cntrl = parseInt($routeParams.id, 10);
          var record = $rootScope.records[cntrl - 1];
 ...
 * anymore.)
 *
 * In addition to the animation promise, we can also make use of animation-related callbacks within our directives and cntrl code by registering
 * an event listener using the `$animate` service. Let's say for example that an animation was triggered on our view
 * routing cntrl to hook into that:
 *
 * ```js
 * ngModule.cntrl('HomePageController', ['$animate', function($animate) {
